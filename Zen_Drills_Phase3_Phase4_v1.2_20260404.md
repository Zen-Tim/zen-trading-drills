# Zen Drills Phase 3 + Phase 4 Build v1.2 · 20260404

## Overview

Two phases, three Claude Code prompts:

- **Phase 3A** — Item management (edit, delete, reorder items within a section) — DONE
- **Phase 3B** — Section management (create, rename, delete, reorder sections) + bugfixes
...
6. **Wrong onConflict in reorderItems (fix in Phase 3B):** `drill_items` has a composite primary key `(id, user_id)` but `reorderItems` upsert uses `onConflict: 'id'`. Should be `'id,user_id'`. Fixed at the top of Phase 3B prompt.

7. **"New Round" button shows in Recent section (fix in Phase 3B):** Recent is a virtual mirror — reshuffling it makes no sense. "New Round" should be hidden in the top bar, checklist bottom, and flashcard controls when `isRecent` is true. `isRecent` also needs to be passed to DrillFlashcard (currently only DrillChecklist has it).

---

## Phase 3A — Item Management — DONE

(Prompt kept for reference — already executed successfully)

<details>
<summary>Click to expand Phase 3A prompt</summary>
...
- Open a section, go to List mode, tap Manage, reorder items — confirm this still works (the onConflict fix).
- Open the Recent section — confirm "New Round" button is NOT visible in top bar, checklist bottom, or flashcard controls.
- The Recent section tile should not show edit/reorder controls.
...
1. ~~Phase 3A prompt → push → test item edit/delete/reorder~~ DONE
2. Phase 3B prompt → push → test section create/edit/delete/reorder + verify reorderItems fix + verify no New Round in Recent
...
| v1.1 | 20260404 | Phase 3A marked done. Added STEP 0 to Phase 3B: fix reorderItems onConflict from 'id' to 'id,user_id'. Added to Known Issues list. |
| v1.2 | 20260404 | Added STEP 0B to Phase 3B: hide "New Round" in Recent section across all three locations (top bar, checklist bottom, flashcard controls). Pass isRecent to DrillFlashcard. Added to Known Issues list. |
